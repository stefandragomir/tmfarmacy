...
class Pharm_Stats_DB(object):
	...
	def read(self,source):
		...
		if not _error:
			...
			_error,_table_name = self.get_source_table(source)

			if not _error:

				#get the sql properties from table to retreive
				_error,_query,_keys = self.get_source_query(source)				
				...
				if not _error:				
					...
					if not _error:
						...
						_error,_data = self.db_rows_to_events(_rows,source,_keys)
